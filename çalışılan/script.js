...
function createDefaultUserData({ name, email, createdDate }) {
    return {
        name: name || 'Kullanici',
        email: email || '',
        ...
        words: [],
        stats: {
            totalWords: 0, masteredWords: 0, reviewsCompleted: 0,
            streak: 0, lastStudyDate: null, studyDates: [],
            totalQuizzes: 0, averageScore: 0, bestStreak: 0,
            todayQuizzes: 0, activities: []
        }
    };
}
...
function isRecoverableCloudError(error) {
    ...
    return error?.code === '42P01' ||
        error?.code === '42501' ||
        (message.includes('relation') && message.includes('user_data')) ||
...
function startLocalBypassSession({ email, name }) {
    const safeEmail = (email || '').trim().toLowerCase();
    const fallbackName = (name || '').trim() || safeEmail.split('@')[0] || 'Kullanici';

    // Local bypass sadece geliştirme için: Supabase doğrulama ve bulut kayıt atlanır.
    cloudSyncDisabled = true;
    currentUser = `local_${safeEmail || Date.now().toString()}`;
    currentUserData = createDefaultUserData({
        name: escapeHtml(fallbackName),
        email: safeEmail,
...
const API_CONFIG = {
    ...
    development: {
        ...
        // ⚠️ UYARI: API key'i .env dosyasından alın!
        // Asla kaynak kodda bırakmayın
        apiKey: '',
        useProxy: false
    }
};
...
// security.js dosyasından yükleniyor
// ============================================
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Supabase hazır olduğunda oturum kontrolü yap
    async function checkSession() {
        ...
        try {
            ...
            if (session && session.user) {
                debugLog('Existing session found, auto-logging in...', { userId: session.user.id });
                
                const cloudData = await loadUserFromCloud(session.user.id);
                currentUser = session.user.id;
                currentUserData = cloudData || createDefaultUserData({
                    name: session.user.user_metadata?.name || session.user.email?.split('@')[0],
                    email: session.user.email,
                    createdDate: session.user.created_at
                });
                if (!cloudData) {
                    await saveUserToCloud(currentUser, currentUserData);
                }
                showMainApp();
                showNotification(`🎉 Tekrar hoş geldin ${escapeHtml(currentUserData.name)}!`, 'success');
            }
        } catch (error) {
    ...
    // Supabase modülü (type=module) async yüklenir, hazır olunca kontrol et
    if (window.supabaseClient) {
        checkSession();
    } else {
        window.addEventListener('supabaseReady', checkSession);
    }
});
...
// ============================================
// ☁️ SUPABASE CLOUD STORAGE FUNCTIONS
// ============================================

// Supabase client'ı al (supabase.js modülünden yükleniyor)
function getSupabase() {
    if (!window.supabaseClient) {
        const reason = window.supabaseStatus?.error ? ` (${window.supabaseStatus.error})` : '';
        console.warn(`⚠️ Supabase client henüz yüklenmedi${reason}`);
        return null;
    }
...
function logSupabaseNetworkDiagnostic(context, error) {
    const rawUrl = window.supabaseStatus?.url || window.SUPABASE_CONFIG?.url || '';
    let supabaseOrigin = 'unknown';
    try {
        supabaseOrigin = rawUrl ? new URL(rawUrl).origin : 'unknown';
    } catch (_) {
        supabaseOrigin = 'invalid_url';
    }

    console.warn('Supabase network diagnostic:', {
        context,
...
async function saveUserToCloud(userId, userData) {
    ...
    if (cloudSyncDisabled) return null;
    
    try {
        debugLog('Saving user data to Supabase', { userId });
        ...
        if (isRecoverableCloudError(error)) {
            cloudSyncDisabled = true;
            showNotification('⚠️ Supabase tablo/izin sorunu: bulut senkronizasyonu kapatıldı.', 'warning');
            return null;
        }
        debugLog('Save to Supabase error', error);
        console.error('Save to Supabase error:', error);
        throw error;
    }
}

// Kullanıcı verisini Supabase'den yükle
async function loadUserFromCloud(userId) {
    ...
    try {
        debugLog('Loading user data from Supabase', { userId });
        ...
        if (error) {
            if (error.code === 'PGRST116') {
                // Row not found — yeni kullanıcı
                debugLog('No data found for user (new user)');
                return null;
            }
            if (isRecoverableCloudError(error)) {
                cloudSyncDisabled = true;
                showNotification('⚠️ Supabase tablo/izin sorunu: yerel veri ile devam ediliyor.', 'warning');
                return null;
            }
            throw error;
        }
        ...
        debugLog('Load from Supabase error', error);
        console.error('Load from Supabase error:', error);
        return null;
    }
}

// Kullanıcı verisini Supabase'de güncelle
async function updateUserInCloud(userId, userData) {
    return saveUserToCloud(userId, userData);
}

// Fisher-Yates shuffle yardımcı fonksiyonu (güvenli, orijinal diziyi değiştirmez)
...
// Authentication Functions - SUPABASE AUTH VERSİYON
async function loginUser() {
    ...
    const sb = getSupabase();
    if (!sb) {
        showNotification('❌ Bağlantı hatası! Sayfayı yenileyin.', 'error');
        return;
    }
    ...
    try {
        ...
        if (authError) {
            debugLog('Supabase auth error', authError);
            if (isFetchFailure(authError)) {
                logSupabaseNetworkDiagnostic('login.auth.signInWithPassword', authError);
                showNotification('❌ Supabase erişim hatası (ağ/CORS). İnterneti, localhost adresini ve proje URL ayarını kontrol edin.', 'error');
            } else if (authError.message.includes('Invalid login credentials')) {
                showNotification('❌ E-mail veya şifre yanlış!', 'error');
            } else if (authError.message.includes('Email not confirmed')) {
                if (isLocalhostRuntime()) {
                    startLocalBypassSession({ email });
                    showNotification('⚠️ Localhost bypass aktif: e-mail doğrulama geçici olarak atlandı (sadece yerel geliştirme).', 'warning');
                } else {
                    showNotification('❌ E-mail adresinizi doğrulamanız gerekiyor. E-postanızı kontrol edin.', 'error');
                }
            } else {
                showNotification(`❌ Giriş hatası: ${authError.message}`, 'error');
            }
            return;
        }

        debugLog('Supabase login successful', { userId: authData.user.id });
        
        // Kullanıcı verisini Supabase'den yükle
        const cloudData = await loadUserFromCloud(authData.user.id);
        
        currentUser = authData.user.id;
        currentUserData = cloudData || createDefaultUserData({
            name: authData.user.user_metadata?.name || email.split('@')[0],
            email: email,
            createdDate: authData.user.created_at
        });
        
        // İlk giriş ise veriyi kaydet
        if (!cloudData) {
            await saveUserToCloud(currentUser, currentUserData);
        }
        
        showMainApp();
        showNotification(`🎉 Hoş geldin ${escapeHtml(currentUserData.name)}!`, 'success');
        
    } catch (error) {
        debugLog('Login error', error);
        console.error('Login error:', error);
        if (isFetchFailure(error)) {
            logSupabaseNetworkDiagnostic('login.catch', error);
            showNotification('❌ Giriş isteği ağda engellendi (Failed to fetch). CORS/CSP veya Supabase erişimini kontrol edin.', 'error');
        } else {
...
async function registerUser() {
    ...
    try {
        ...
        if (authError) {
            debugLog('Supabase register error', authError);
            // Tam hata mesajını hem bildirim hem alert ile göster
            const errMsg = authError.message || JSON.stringify(authError);
            if (isFetchFailure(authError)) {
                logSupabaseNetworkDiagnostic('register.auth.signUp', authError);
                showNotification('❌ Supabase kayıt isteği engellendi (Failed to fetch). Ağ/CORS/CSP veya proje URL/anon key ayarını kontrol edin.', 'error');
            } else {
                showNotification(`❌ Kayıt hatası: ${errMsg}`, 'error');
            }
            alert(`Supabase Kayıt Hatası:\n${errMsg}`);
            return;
        }

        debugLog('Supabase registration successful', { userId: authData.user?.id });
        ...
        // Eğer e-mail doğrulama aktifse kullanıcıyı bilgilendir
        if (authData.user && !authData.session) {
            if (isLocalhostRuntime()) {
                startLocalBypassSession({
                    email,
                    name: escapeHtml(name)
                });
                showNotification('⚠️ Localhost bypass aktif: hesap doğrulaması geçici olarak atlandı (sadece yerel geliştirme).', 'warning');
            } else {
                showNotification('📧 Kayıt başarılı! E-postanızı kontrol edip hesabınızı doğrulayın.', 'success');
            }
            return;
        }

        // E-mail doğrulama kapalıysa direkt giriş yap
        if (authData.user && authData.session) {
            currentUser = authData.user.id;
            ...
            showMainApp();
            showNotification(`🎉 Hesabın oluşturuldu! Hoş geldin ${escapeHtml(name)}!`, 'success');
        ...
        if (isFetchFailure(error)) {
            logSupabaseNetworkDiagnostic('register.catch', error);
        }
        alert(`Beklenmeyen Kayıt Hatası:\n${errMsg}`);
        if (isFetchFailure(error)) {
            showNotification('❌ Kayıt isteği ağda başarısız oldu (Failed to fetch). CORS/CSP ve Supabase erişimini kontrol edin.', 'error');
        } else {
...
function showMainApp() {
    ...
    setTimeout(() => {
        ...
        if (dueWords.length > 0 && 'Notification' in window && Notification.permission === 'granted') {
...
async function logout() {
    ...
    if (sb?.auth?.signOut) {
        try {
            ...
            showNotification('⚠️ Oturum kapatılırken ağ hatası oluştu, yerel oturum kapatıldı.', 'warning');
        }
    }
...
function updateNotifications() {
    ...
    if ('Notification' in window && !hasNotificationPermission && Notification.permission !== 'denied') {
        notificationsHTML += `
...
function updateAchievementBadges() {
    ...
    badges.push({ icon: totalQuizzes >= 2000 ? '💫' : '🔒', title: t('badge_quizEmperor'), unlocked: totalQuizzes >= 2000, progress: `${Math.min(totalQuizzes, 2000)}/2000` });
                            ...
                            <div class="p-3 border border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                                <div class="flex items-center justify-between">
                                    <div class="flex-1">
                                        ...
                                        <p class="text-xs text-gray-500">📁 ${folder ? escapeHtml(folder.name) : 'Bilinmeyen Klasör'}</p>
                                    </div>
                                    <div class="flex gap-2 ml-4">
                                        <button onclick="editWord('${escapeJsString(word.id)}')" class="px-2 py-1 bg-blue-500 text-white rounded text-xs hover:bg-blue-600" title="${t('edit')}">
                                            ✏️
                                        </button>
                                        <button onclick="deleteWord('${escapeJsString(word.id)}')" class="px-2 py-1 bg-red-500 text-white rounded text-xs hover:bg-red-600" title="${t('delete')}">
                                            🗑️
                                        </button>
                                    </div>
                                </div>
...
function renderFolderHierarchy(folders, level = 0) {
    return folders.map(folder => {
        ...
        return `
            <div class="folder-item" style="margin-left: ${Math.max(0, Number(level) || 0) * 20}px;">
                <div class="bg-white p-4 border border-gray-200 rounded-lg mb-3 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ${hasSubFolders ? 
                                `<button onclick="toggleFolder('${escapeJsString(folder.id)}')" class="text-lg text-gray-600 hover:text-gray-800">
                                    ${isExpanded ? '▼' : '▶'}
                                </button>` : 
                                '<span class="w-6"></span>'
                            }
                            ...
                            <div>
                                <h3 class="font-semibold text-gray-800">${escapeHtml(folder.name)}</h3>
                                <p class="text-xs text-gray-500">${wordCount} ${t('words')} ${hasSubFolders ? `• ${subFolders.length} ${t('subFolders')}` : ''}</p>
                            </div>
                        </div>
                        <div class="flex gap-2">
                            <button onclick="addSubFolder('${escapeJsString(folder.id)}')" class="px-3 py-1 bg-green-500 text-white rounded text-sm hover:bg-green-600" title="${t('addSubFolder')}">
                                ➕
                            </button>
                            <button onclick="openFolder('${escapeJsString(folder.id)}')" class="px-3 py-1 bg-blue-500 text-white rounded text-sm hover:bg-blue-600" title="${t('open')}">
                                👁️
                            </button>
                            <button onclick="deleteFolder('${escapeJsString(folder.id)}')" class="px-3 py-1 bg-red-500 text-white rounded text-sm hover:bg-red-600" title="${t('delete')}">
                                🗑️
                            </button>
                        </div>
                    </div>
    ...
    modal.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
            ...
            <div class="flex gap-3">
                <button onclick="createFolder('${escapeJsString(parentId || '')}')" class="flex-1 px-4 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-semibold">
                    ${t('create')}
                </button>
...
function renderFolderWords(folderId) {
    ...
    foldersList.innerHTML = `
        <div class="mb-6 flex items-center justify-between">
            ...
            <button onclick="showAddWordModal('${escapeJsString(folderId)}')" class="flex items-center px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors">
                ➕ ${t('addWord')}
            </button>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center mb-6">
                <span class="text-3xl mr-3">📁</span>
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">${escapeHtml(folder.name)}</h2>
            ...
            ${folderWords.length === 0 ? `
                <div class="text-center py-12">
                    ...
                    <button onclick="showAddWordModal('${escapeJsString(folderId)}')" class="px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600">
                        ➕ ${t('addWord')}
                    </button>
                </div>
            ` : `
                    ...
                    ${folderWords.map(word => `
                        <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center mb-2">
                                        <h3 class="font-bold text-lg text-gray-800 mr-3">${escapeHtml(word.word)}</h3>
                                        <span class="px-2 py-1 text-xs rounded-full ${
                                            word.difficulty === 'easy' ? 'bg-green-100 text-green-800' :
                                            word.difficulty === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                                            'bg-red-100 text-red-800'
                                    ...
                                    <p class="text-gray-700 mb-2">${escapeHtml(word.definition)}</p>
                                ...
                                <div class="flex gap-2 ml-4">
                                    <button onclick="editWord('${escapeJsString(word.id)}')" class="px-3 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" title="${t('edit')}">
                                        ✏️
                                    </button>
                                    <button onclick="deleteWord('${escapeJsString(word.id)}')" class="px-3 py-2 bg-red-500 text-white rounded hover:bg-red-600" title="${t('delete')}">
                                        🗑️
                                    </button>
                                </div>
                            </div>
    ...
    modal.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md" id="wordModal">
            ...
            <div class="flex gap-3 mt-6">
                <button onclick="saveWord('${escapeJsString(folderId)}')" class="flex-1 px-4 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 font-semibold">
                    ${t('save')}
                </button>
                <button onclick="closeModal()" class="flex-1 px-4 py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 font-semibold">
                    ${t('cancel')}
...
function editWord(wordId) {
    ...
    modal.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md" id="wordModal">
            ...
            <div class="space-y-4">
                <input type="text" id="wordInput" value="${escapeHtml(word.word)}" placeholder="${t('word')}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                <textarea id="definitionInput" placeholder="${t('definition')}" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">${escapeHtml(word.definition)}</textarea>
                <select id="difficultyInput" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div class="flex gap-3 mt-6">
                <button onclick="saveWord('${escapeJsString(word.folderId)}', '${escapeJsString(wordId)}')" class="flex-1 px-4 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-semibold">
                    ${t('save')}
                </button>
                <button onclick="closeModal()" class="flex-1 px-4 py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 font-semibold">
                    ${t('cancel')}
...
function deleteFolder(folderId) {
    ...
    // Klasördeki ve tüm alt klasörlerdeki kelimeleri sil (recursive)
    if (userData.words) {
        const allFolderIds = collectAllFolderIds(folder);
        userData.words = userData.words.filter(word => !allFolderIds.includes(word.folderId));
    }
...
// Bir klasörün ve tüm alt klasörlerinin ID'lerini topla
function collectAllFolderIds(folder) {
    if (!folder) return [];
    let ids = [folder.id];
    if (folder.subFolders) {
        for (const sub of folder.subFolders) {
            ids = ids.concat(collectAllFolderIds(sub));
        }
    }
    return ids;
}
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                        <p><strong>${t('name')}:</strong> ${escapeHtml(currentUserData.name || '')}</p>
                        <p><strong>${t('email')}:</strong> ${escapeHtml(currentUserData.email || '')}</p>
...
function highlightSearchTerm(text) {
    ...
    if (!currentSearchTerm || !normalizedText) return escapeHtml(normalizedText);
    
    // Regex özel karakterlerini escape et (ReDoS/XSS koruması)
...
function updateQuizStats() {
    ...
    if (!userData.stats) {
        userData.stats = {
            ...
            bestStreak: 0,
            todayQuizzes: 0,
            todayQuizzesDate: null
        };
    }
    
    // Günlük quiz sayacını sıfırla (gün değiştiyse)
    const today = new Date().toISOString().split('T')[0];
    if (userData.stats.todayQuizzesDate !== today) {
        userData.stats.todayQuizzes = 0;
        userData.stats.todayQuizzesDate = today;
    }
...
function startQuickQuiz() {
    ...
    // 5 rastgele kelime seç (Fisher-Yates shuffle - orijinal diziyi mutasyondan korur)
...
function startDifficultyQuiz() {
    ...
    currentQuiz = shuffleArray(hardWords);
    currentQuizIndex = 0;
    quizScore = 0;
...
function startReviewQuiz() {
    ...
    currentQuiz = shuffleArray(reviewWords);
    currentQuizIndex = 0;
    quizScore = 0;
...
function startMasteryQuiz() {
    ...
    currentQuiz = shuffleArray(masteredWords).slice(0, 10);
    currentQuizIndex = 0;
    quizScore = 0;
...
function showMultipleChoiceQuestion() {
    ...
    const wrongOptions = shuffleArray(allWords).slice(0, 3);
    ...
    quizGame.innerHTML = `
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="text-center mb-8">
                <h2 class="text-3xl font-bold text-gray-800 mb-4">${escapeHtml(word.word)}</h2>
                <p class="text-lg text-gray-600 mb-6">${t('selectCorrectAnswer')}</p>
                
                <div class="space-y-3">
                    ${options.map((option, index) => `
                        <button onclick="checkMultipleChoiceAnswer('${escapeJsString(option.id)}', '${escapeJsString(word.id)}')" 
                            ...
                            <span class="ml-3 text-gray-800">${escapeHtml(option.definition)}</span>
                        </button>
                    `).join('')}
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-center mb-8">
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-4">
                    <h3 class="font-bold text-lg text-gray-800 mb-2">${escapeHtml(word.word)}</h3>
                    <p class="text-gray-700"><strong>${t('correctAnswer')}:</strong> ${escapeHtml(word.definition)}</p>
                </div>
...
function showSpellingQuestion() {
    ...
    const scrambledWord = shuffleArray(word.word.split('')).join('');
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="text-center mb-8">
                <div class="bg-blue-50 p-4 rounded-lg mb-6">
                    ...
                    <p class="text-xl font-semibold text-blue-800">${escapeHtml(word.definition)}</p>
                </div>
                ...
                <div id="spellingHint" class="hidden bg-yellow-50 p-3 rounded-lg mb-4">
                    <p class="text-sm text-yellow-800">💡 Harfler: <span class="font-mono font-bold">${escapeHtml(scrambledWord)}</span></p>
    ...
    quizGame.innerHTML = `
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-center mb-8">
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-4">
                    <p class="text-gray-700 mb-2"><strong>Anlam:</strong> ${escapeHtml(word.definition)}</p>
                    <p class="text-lg font-bold text-green-600 mb-2"><strong>Doğru Yazım:</strong> ${escapeHtml(word.word)}</p>
                    <p class="text-gray-600"><strong>Sizin Yazımınız:</strong> ${escapeHtml(userAnswer)}</p>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="text-center mb-8">
                <div class="bg-gradient-to-r from-teal-50 to-blue-50 p-6 rounded-lg mb-6">
                    ...
                    <h2 class="text-2xl font-bold text-gray-800 mb-4">${escapeHtml(word.word)}</h2>
                    <button onclick="playWordAudio('${escapeJsString(word.word)}')" class="px-6 py-3 bg-teal-500 text-white rounded-lg hover:bg-teal-600 font-semibold mb-4">
                        ${t('playAudio')}
                    </button>
                </div>
    ...
    quizGame.innerHTML = `
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-center mb-8">
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-4">
                    <h3 class="font-bold text-lg text-gray-800 mb-2">${escapeHtml(word.word)}</h3>
                    <p class="text-gray-700 mb-2"><strong>${t('correctAnswer')}:</strong> ${escapeHtml(word.definition)}</p>
                    <p class="text-gray-600"><strong>${t('yourAnswer')}:</strong> ${escapeHtml(userAnswer)}</p>
                    ${hasTypo ? `<p class="text-orange-600 text-sm mt-2">⚠️ Küçük yazım hatası tespit edildi ama cevabın doğru sayıldı!</p>` : ''}
                    <button onclick="playWordAudio('${escapeJsString(word.word)}')" class="mt-2 px-4 py-2 bg-teal-500 text-white rounded-lg hover:bg-teal-600">
                        🔊 Tekrar Dinle
                    </button>
                </div>
...
function renderFolderOptions(folders, level = 0) {
    return folders.map(folder => {
        ...
        let html = `
            <button onclick="startQuizFromFolder('${escapeJsString(folder.id)}')" class="w-full text-left p-3 hover:bg-gray-100 rounded-lg border border-gray-200">
                ${indent}📁 ${escapeHtml(folder.name)} (${wordCount} kelime)
            </button>
        `;
...
function startQuizFromFolder(folderId) {
    ...
    currentQuiz = shuffleArray(folderWords);
    currentQuizIndex = 0;
    quizScore = 0;
    ...
    quizGame.innerHTML = `
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="text-center mb-8">
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-4">
                    <h3 class="font-bold text-lg text-gray-800 mb-2">${escapeHtml(word.word)}</h3>
                    <p class="text-gray-700 mb-2"><strong>${t('correctAnswer')}:</strong> ${escapeHtml(correctAnswer)}</p>
                    <p class="text-gray-600"><strong>${t('yourAnswer')}:</strong> ${escapeHtml(userAnswer)}</p>
                    ${hasTypo ? `<p class="text-orange-600 text-sm mt-2">⚠️ Küçük yazım hatası tespit edildi ama cevabın doğru sayıldı!</p>` : ''}
                </div>
    ...
    quizGame.innerHTML = `
        <div class="bg-white rounded-lg shadow-md p-6 text-center">
            ...
            <div class="flex justify-center gap-4">
                ...
                ${currentQuizFolder ? `
                    <button onclick="startQuizFromFolder('${escapeJsString(currentQuizFolder)}')" class="px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 font-semibold">
                        ${t('sameFolder')}
                    </button>
                ` : ''}
            </div>
...
function populateStatsFilters() {
    ...
    // Eski listener'ı temizle (memory leak'i önle)
    ...
    newFolderSelect.addEventListener('change', function() {
